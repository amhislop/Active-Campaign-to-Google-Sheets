const utilities = {
  ...
  formatDate(date) {
    ...
    const yyyy = dateTime.getFullYear(),
      ...
      min = utilities.leadingZeros(dateTime.getMinutes()),
      ss = utilities.leadingZeros(dateTime.getSeconds());
  ...
  getRelativeColumnIndex(arr, currentIndex, query) {
    return arr.findIndex(({ column }) => column === query) - currentIndex;
  },

  getIndirectPath({ fieldMap, index }, query) {
    if (query.includes('/')) {
      const [queryOne, queryTwo] = query
        .split('/')
        .map((q) => utilities.getRelativeColumnIndex(fieldMap, index, q));
      return `=sum(INDIRECT("R[0]C[${queryOne}]", FALSE)/INDIRECT("R[0]C[${queryTwo}]", FALSE))`;
    } else {
      const relativeIndex = utilities.getRelativeColumnIndex(
        fieldMap,
        index,
        query
      );
